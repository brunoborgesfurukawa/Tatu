...
abstract class Record {
	...
	/**
	 * Array cujos índices representam os campos da tabela no banco de dados.
	 *
	 * Este array é preenchido na criação do objeto (função __construct).
	 */
	protected $_data;
	/**
	 * Array utilizado para fazer cache de valores retornados pelo método
	 * __get.
	 *
	 * Toda vez que algum método get* é chamado (através do método mágico
	 * __get), o valor retornado pelo método é armazenado neste array, evitando
	 * que o método seja reexecutado a cada acesso ao atributo.
	 */
	protected $_cache;
	/**
	 * Array com lista de campos da tabela, usado para armazenar a consulta
	 * feita pelo método tableFields().
	 */
	...
	function __get($prop) {
		if (array_key_exists($prop, $this->_cache)) {
			...
			error_log('Tentando acessar uma propriedade inexistente: ' . get_class($this) . '->' . $prop . ' em ' . $linha);
		}
	}
