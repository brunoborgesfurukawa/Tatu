...
if (isset($_GET['id'])) {
	...
	// busca as Pessoas
	$pessoas = Pessoa::find(
		array('nome LIKE ? AND aprovacao = 0', '%' . $busca . '%'),
		array(
			'select' => 'Pessoas.codPessoa, nome, dataNascimento, codColaborador',
			'joins' => 'INNER JOIN Contatos ON Pessoas.codPessoa = Contatos.codPessoa',
			'order' => 'nome',
			'limit' => $limite
		)
	);
	...
	foreach ($pessoas as $pessoa) {
		$linha = array();
		$linha['colaborador'] = $pessoa->codColaborador;
